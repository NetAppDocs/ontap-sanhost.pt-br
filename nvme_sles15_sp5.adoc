...
[role="lead"]
O NVMe sobre Fabrics (NVMe-of), incluindo NVMe sobre Fibre Channel (NVMe/FC) e outros transportes, é compatível com o SUSE Linux Enterprise Server (SLES) 15 SP5 com acesso a namespace assimétrico (ANA). Em ambientes NVMe-of, o ANA é equivalente ao multipathing ALUA em ambientes iSCSI e FCP e é implementado com multipath NVMe no kernel.

O suporte a seguir está disponível para a configuração de host NVMe-of para SLES 15 SP5 com ONTAP:
...
* Não `sanlun` há suporte para NVMe-of. Portanto, o suporte ao utilitário host não está disponível para NVMe-of em um host SLES 15 SP5. Você pode confiar no plug-in do NetApp incluído no pacote nvme-cli nativo para todos os transportes NVMe-of.
...
--
.Passos
. O driver nativo da caixa de entrada qla2xxx incluído no kernel SLES 15 SP5 tem as correções mais recentes essenciais para o suporte ao ONTAP. Verifique se você está executando o driver de adaptador e as versões de firmware compatíveis:
+
[listing]
----
...
A partir do ONTAP 9.11,1, você pode criar um controlador de descoberta persistente (PDC) para o seu host SLES 15 SP5 usando o seguinte procedimento. Um PDC é necessário para detectar automaticamente o subsistema NVMe, adicionar ou remover cenários e alterações aos dados da página de log de descoberta.
...
A partir do ONTAP 9.12,1, a autenticação segura na banda é compatível com NVMe/TCP e NVMe/FC entre o seu host SLES 15 SP5 e a controladora ONTAP.
...
----
. Gerar a chave dhchap para o host SLES15 SP5:
+
[listing]
----
...
Não há problemas conhecidos para o SLES 15 SP5 com a versão ONTAP.
