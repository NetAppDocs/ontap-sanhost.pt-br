...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.6. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rockylinux-supported-features.html["Suporte e recursos do Rocky Linux ONTAP"].
...
.Passos
. Instale o Rocky Linux 9.6 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel Rocky Linux 9.6 necessário:
+
[source, cli]
----
uname -r
