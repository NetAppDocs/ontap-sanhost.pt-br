...
summary: Descreve como usar o Oracle Linux 8.x com o ONTAP. 
---
= Configurar o Oracle Linux 8.x para FCP e iSCSI com armazenamento ONTAP.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
O software Linux Host Utilities fornece ferramentas de gerenciamento e diagnóstico para hosts Linux conectados ao armazenamento ONTAP . Ao instalar os utilitários de host Linux em um host Oracle Linux 8.x, você pode usá-los para gerenciar operações de protocolo FCP e iSCSI com LUNs ONTAP .
...
Você pode usar o recurso de multipathing com o Oracle Linux 8.x para gerenciar LUNs do ONTAP .


NOTE: Você pode usar o link:hu-rhel-8x.html#rhel-rhck["Configurações recomendadas para Red Hat Enterprise Linux (RHEL) 8.x"] Configurar o kernel compatível com Red Hat para Oracle Linux 8.x.
...
Se o seu host estiver conectado a LUNs de outros fornecedores e qualquer uma das configurações de parâmetros de multipath estiver sobrescrita, você precisa corrigi-las modificando uma seção existente ou adicionando uma nova seção no arquivo  `multipath.conf` que se aplique especificamente às LUNs ONTAP. Você precisa rolar para baixo no arquivo para revisar as alterações que se aplicam ao seu host. Se você não corrigir os parâmetros, as LUNs ONTAP podem não funcionar como esperado.
