...
O NVMe/FC é compatível com o ONTAP 9.6 ou posterior para as seguintes versões do SLES:

* SLES15 SP1
+
O host SLES15 SP1 pode executar o tráfego NVMe/FC e FCP nas mesmas portas do adaptador do iniciador de fibra. Consulte o link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] para obter uma lista de controladores e adaptadores FC compatíveis.
...
* Os scripts nativos de conexão automática NVMe/FC estão incluídos no pacote nvme-cli. Você pode usar o driver lpfc nativo da caixa de entrada no SLES15 SP1.
...
----
. No host SLES15 SP1, verifique a string NQN do host em `/etc/nvme/hostnqn` e verifique se ela corresponde à string NQN do host para o subsistema correspondente no array ONTAP. Por exemplo:
+
[listing]
----
