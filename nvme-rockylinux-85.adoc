...
summary: Como configurar o host NVMe-oF para Rocky Linux 8.5 com ONTAP 
---
= Configurar o Rocky Linux 8.5 com NVMe-oF para armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Os hosts Rocky Linux 8.5 suportam os protocolos NVMe/FC e NVMe/TCP com Acesso Assimétrico ao Namespace (ANA). ANA é equivalente ao multipathing de acesso assimétrico à unidade lógica (ALUA) em ambientes iSCSI e FCP e é implementado usando o recurso multipath NVMe no kernel.
...
.Sobre esta tarefa
Você pode usar o seguinte suporte e recursos com a configuração do host NVMe-oF para o Rocky Linux 8.5. Você também deve rever as limitações conhecidas antes de iniciar o processo de configuração.
...
+
** O multipath NVMe no kernel é desabilitado por padrão para hosts Rocky Linux 8.5 NVMe-oF. Portanto, você precisa ativá-lo manualmente.
** Em hosts Rocky Linux 8.5, NVMe/TCP é um recurso de prévia de tecnologia devido a problemas em aberto. Consulte https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/8.5_release_notes/index#technology-preview_file-systems-and-storage["Notas de lançamento do Rocky Linux 8.5"^] para obter mais informações.
...
Use o procedimento a seguir para validar as versões mínimas suportadas do software Rocky Linux 8.5.

.Passos
. Instale o Rocky Linux 8.5 no servidor. Após a conclusão da instalação, verifique se você está executando o kernel especificado do Rocky Linux 8.5:
+
[source, cli]
----
uname -r
...
Não há problemas conhecidos para a configuração do host NVMe-oF no Rocky Linux 8.5 com lançamento ONTAP.
