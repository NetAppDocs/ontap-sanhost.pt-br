...
summary: Configurando a configuração de host VME/FC para Oracle Linux 8,1 com ONTAP, com exemplos 
---
= Configuração de host NVMe/FC para Oracle Linux 8,1 com ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
* Por padrão, o balanceamento de carga round-robin não é habilitado no NVMe Multipath. Você deve escrever uma regra udev para habilitar essa funcionalidade. As etapas são fornecidas na seção sobre como habilitar o NVMe/FC no Oracle Linux 8,1.
* Não há suporte sanlun para NVMe/FC e, como consequência, não há suporte a utilitários de host Linux para NVMe/FC no Oracle Linux 8,1. Use a saída do comando ONTAP disponível como parte do plug-in NetApp incluído no nvme-cli nativo.
...
. Instale o Oracle Linux 8,1 no servidor.
...
----
. No host Oracle Linux 8,1, verifique a string NQN do host em /etc/nvme/hostnqn e verifique se ela corresponde à string NQN do host para o subsistema correspondente no array ONTAP.
+
[listing]
----
...
----
+
No exemplo acima, dois namespaces são mapeados para o host Oracle Linux 8,1 ANA. Elas são visíveis por meio de quatro LIFs de destino: Duas LIFs de nó local e duas outras LIFs de nó parceiro/remoto. Esta configuração mostra como dois caminhos ANA otimizados e dois caminhos ANA inacessíveis para cada namespace no host.
