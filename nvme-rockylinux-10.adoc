...
summary: Como configurar o host NVMe-oF para Rocky Linux 10.0 com armazenamento ONTAP 
---
= Configurar o Rocky Linux 10.0 para NVMe-oF com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 10.0. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rockylinux-supported-features.html["Suporte e recursos do Rocky Linux ONTAP"].

*NVMe-oF com Rocky Linux 10.0 tem as seguintes limitações conhecidas:*
...
.Passos
. Instale o Rocky Linux 10.0 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel necessário do Rocky Linux 10.0:
+
[source, cli]
----
uname -r
...
A partir do ONTAP 9.12.1, a autenticação segura em banda é suportada por NVMe/TCP entre um host Rocky Linux 10.0 e um controlador ONTAP .
...
cat /etc/nvme/hostnqn
----
. Gere a chave dhchap para o host Rocky Linux 10.0.
+
