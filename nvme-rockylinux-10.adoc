...
Com o Rocky Linux 10, o  `nvmefc-boot-connections.service` e  `nvmf-autoconnect.service` serviços de inicialização incluídos no NVMe/FC  `nvme-cli` Os pacotes são ativados automaticamente quando o sistema é inicializado. Após a inicialização, verifique se os serviços estão habilitados.
