...
summary: Como configurar um host NVMe-oF para RHEL 8.x com ONTAP 
---
= Configure o RHEL 8.x para NVMe-oF com armazenamento ONTAP.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para RHEL 8.x. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rhel-supported-features.html["Suporte e recursos do RHEL ONTAP"].

O NVMe-oF com RHEL 8.x apresenta as seguintes limitações conhecidas:

* A inicialização SAN usando o protocolo NVMe-oF não é suportada atualmente.
* O recurso de multipath NVMe no kernel está desativado por padrão em hosts NVMe-oF no RHEL 8.x; você precisa ativá-lo manualmente.
...
.Passos
. Instale o RHEL 8.x no servidor. Após a conclusão da instalação, verifique se você está executando o kernel RHEL 8.x necessário:
+
[source, cli]
----
uname -r
...
----
. No host RHEL 8.x, verifique o `hostnqn` corda em `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Os hosts RHEL 8.x NVMe-oF criam controladores de descoberta persistentes (PDCs) duplicados. | Em hosts NVMe-oF, você pode usar o comando "nvme discover -p" para criar PDCs. Ao utilizar este comando, apenas um PDC deve ser criado por combinação iniciador-alvo. No entanto, se você estiver executando o RHEL 8.x em um host NVMe-oF, um PDC duplicado será criado cada vez que o comando "nvme discover -p" for executado. Isso leva ao uso desnecessário de recursos tanto no host quanto no destino. 
|===