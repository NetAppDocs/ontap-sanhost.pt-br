...
summary: Como configurar o host NVMe-oF para Rocky Linux 9.1 com ONTAP 
---
= Configurar o Rocky Linux 9.1 com NVMe-oF para armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Os hosts Rocky Linux 9.1 suportam os protocolos NVMe/FC e NVMe/TCP com Acesso Assimétrico ao Namespace (ANA). ANA é equivalente ao multipathing de acesso assimétrico à unidade lógica (ALUA) em ambientes iSCSI e FCP e é implementado usando o recurso multipath NVMe no kernel.
...
.Sobre esta tarefa
Você pode usar o seguinte suporte e recursos com a configuração do host NVMe-oF para o Rocky Linux 9.1. Você também deve rever as limitações conhecidas antes de iniciar o processo de configuração.
...
+
** O Rocky Linux 9.1 habilita multicaminhos NVMe no kernel para namespaces NVMe por padrão, eliminando a necessidade de configurações explícitas.
...
Use o procedimento a seguir para validar as versões mínimas suportadas do software Rocky Linux 9.1.

.Passos
. Instale o Rocky Linux 9.1 no servidor. Após a conclusão da instalação, verifique se você está executando o kernel especificado do Rocky Linux 9.1:
+
[source, cli]
----
uname -r
...
A configuração do host NVMe-oF para Rocky Linux 9.1 com armazenamento ONTAP tem o seguinte problema conhecido:
...
| 1503468 | `nvme list-subsys` o comando retorna controladores NVMe repetidos listados para um determinado subsistema | O `nvme list-subsys` comando deve retornar uma lista exclusiva de controladores NVMe associados a um determinado subsistema. No Rocky Linux 9.1, o  `nvme list-subsys` O comando retorna controladores NVMe com seus respectivos estados ANA para todos os namespaces pertencentes a um determinado subsistema. No entanto, o estado ANA é um atributo por namespace; portanto, entradas exclusivas de controladores NVMe com o estado do caminho devem ser exibidas se você listar a sintaxe de comando do subsistema para um determinado namespace. 
|===