...
[role="lead"]
Hosts Rocky Linux 9.4 suportam os protocolos NVMe/FC e NVMe/TCP com Acesso Assimétrico ao Namespace (ANA). ANA é equivalente ao multipathing de acesso assimétrico à unidade lógica (ALUA) em ambientes iSCSI e FCP e é implementado usando o recurso multipath NVMe no kernel.
...
.Sobre esta tarefa
Você pode usar o seguinte suporte e recursos com a configuração do host NVMe-oF para o Rocky Linux 9.4. Você também deve rever as limitações conhecidas antes de iniciar o processo de configuração.
...
+
** O Rocky Linux 9.4 habilita multicaminhos NVMe no kernel para namespaces NVMe por padrão, eliminando a necessidade de configurações explícitas.
...
Use o procedimento a seguir para validar as versões mínimas suportadas do software Rocky Linux 9.4.
