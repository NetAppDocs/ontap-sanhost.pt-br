...
A versão SLES 15 SP2 com ONTAP tem os seguintes problemas conhecidos:
...
| link:https://mysupport.netapp.com/NOW/cgi-bin/bol?Type=Detail&Display=1308744["1308744"^] | O arranque iSCSI a partir de SAN falha ao arrancar com uma configuração IP estática após concluir uma instalação do SO SLES15SP2  a| 
O iSCSI Sanbooted LUN falhou ao inicializar depois de concluir uma instalação do SO SLES 15 SP2 com uma configuração IP estática. A falha de inicialização ocorre toda vez com a configuração de IP estático. Isso leva o servidor a recusar continuar o processo de inicialização com a seguinte mensagem de erro:
