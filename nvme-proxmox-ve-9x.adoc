...
summary: Como configurar um host NVMe-oF para Proxmox VE 9.0 com armazenamento ONTAP 
---
= Configure o Proxmox VE 9.0 para NVMe-oF com armazenamento ONTAP.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
O host Proxmox VE 9.0 suporta os protocolos NVMe over Fibre Channel (NVMe/FC) e NVMe over TCP (NVMe/TCP) com Acesso Assimétrico ao Espaço de Nomes (ANA). O ANA fornece funcionalidade de múltiplos caminhos equivalente ao acesso de unidade lógica assimétrica (ALUA) em ambientes iSCSI e FCP.

Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Proxmox VE 9.0. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-proxmox-supported-features.html["Suporte e recursos do ONTAP"^].

O NVMe-oF com Proxmox VE 9.0 apresenta a seguinte limitação conhecida:
...
== Passo 1: Verifique a versão do software e a configuração do NVMe.
...
.Passos
. Instale o Proxmox VE 9.0 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel Proxmox VE 9.0 necessário:
+
[source, cli]
----
uname -r
----
+
Exemplo de versão do kernel Proxmox VE 9.0:
