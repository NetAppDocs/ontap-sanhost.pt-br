...
A partir do Proxmox VE 9.0, a política de E/S padrão para NVMe-oF é definida como  `queue-depth`. Se você deseja alterar a política de E/S para  `round-robin`, que também está disponível como uma opção configurável, modifique o arquivo de regras udev da seguinte forma:
...
----
. Modifique a regra para que  `queue-depth` seja alterada para  `round-robin`, como mostrado no exemplo a seguir:
+
[listing]
----
...
----
. Verifique a política de iopolicy atual do seu subsistema. Substitua <subsystem> pelo nome do seu subsistema, por exemplo, nvme-subsys0.
+
[source, cli]
----
...
.Exibir exemplo de saída AFF, FAS ou ASA
[%collapsible]
=====
[listing, subs="+quotes"]
----
