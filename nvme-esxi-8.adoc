...
* A partir do ONTAP 9.16.1, a alocação é habilitada por padrão para todos os namespaces NVMe recém-criados.
* A partir do ONTAP 9.9.1 P3, o protocolo NVMe/FC é suportado para o ESXi 8.
* A partir da ONTAP 9.10.1, o protocolo NVMe/TCP é compatível com a ONTAP.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1420654["1420654"^] | O nó ONTAP não está operacional quando o protocolo NVMe/FC é usado com a versão do ONTAP 9.9.1 | ONTAP 9.9.1 introduziu suporte para o comando "abort" do NVMe. Quando ONTAP recebe o comando "abort" para abortar um comando NVMe fundido que está aguardando seu comando correspondente, ocorre uma interrupção no nó do ONTAP. O problema é percebido apenas em hosts que utilizam comandos NVMe fundidos (por exemplo, ESX) e transporte Fibre Channel (FC). 


| link:https://jira.ngage.netapp.com/browse/CONTAPEXT-1126["CONTAPEXT-1126"^] | O erro de e/S ocorre quando as VMs Linux que usam adaptadores vNVMe encontram uma janela longa de todos os caminhos para baixo (APD)  a| 
As máquinas virtuais Linux executando vSphere 8.x e posteriores e usando adaptadores NVMe virtuais (vNVMe) encontram um erro de E/S porque a operação de repetição do vNVMe está desativada por padrão. Para evitar interrupções em máquinas virtuais Linux com kernels mais antigos durante uma falha de todos os caminhos (APD) ou uma carga de E/S intensa, a VMware introduziu um parâmetro configurável "VSCSIDisableNvmeRetry" para desativar a operação de repetição do vNVMe.
