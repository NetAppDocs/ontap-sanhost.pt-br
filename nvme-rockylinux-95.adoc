...
summary: Como configurar o host NVMe-oF para Rocky Linux 9.5 com ONTAP 
---
= Configurar o Rocky Linux 9.5 com NVMe-oF para armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Os hosts Rocky Linux 9.5 suportam os protocolos NVMe/FC e NVMe/TCP com Acesso Assimétrico ao Namespace (ANA). ANA é equivalente ao multipathing de acesso assimétrico à unidade lógica (ALUA) em ambientes iSCSI e FCP e é implementado usando o recurso multipath NVMe no kernel.
...
.Sobre esta tarefa
Você pode usar o seguinte suporte e recursos com a configuração do host NVMe-oF para o Rocky Linux 9.5. Você também deve rever as limitações conhecidas antes de iniciar o processo de configuração.
...
+
** A partir do ONTAP 9.12.1, você pode usar autenticação segura em banda para NVMe-oF com o Rocky Linux 9.5.
** O Rocky Linux 9.5 habilita multicaminhos NVMe no kernel para namespaces NVMe por padrão, eliminando a necessidade de configurações explícitas.
...
Use o procedimento a seguir para validar as versões mínimas suportadas do software Rocky Linux 9.5.

.Passos
. Instale o Rocky Linux 9.5 no servidor. Após a conclusão da instalação, verifique se você está executando o kernel especificado do Rocky Linux 9.5:
+
[source, cli]
----
uname -r
