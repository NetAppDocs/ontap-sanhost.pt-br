...
---
= Configurar o Rocky Linux 9.5 para NVMe-oF com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.5. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rockylinux-supported-features.html["Suporte e recursos do Rocky Linux ONTAP"].

O NVMe-oF com Rocky Linux 9.5 não tem limitações conhecidas.
...
.Passos
. Instale o Rocky Linux 9.5 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel Rocky Linux 9.5 necessário:
+
[source, cli]
----
uname -r
