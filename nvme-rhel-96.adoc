...
----
. No host RHEL, verifique a string hostnqn em  `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
Com o RHEL 9,6, o  `nvmefc-boot-connections.service` e  `nvmf-autoconnect.service` serviços de inicialização incluídos no NVMe/FC  `nvme-cli` Os pacotes são habilitados automaticamente quando o sistema inicializa. Após a inicialização, verifique se os serviços estão habilitados.
