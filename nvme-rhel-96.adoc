...
---
= Configurar RHEL 9.6 para NVMe-oF com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe sobre Fabrics (NVMe-oF) para o Red Hat Enterprise Linux (RHEL) 9.6.  Para obter mais informações sobre suporte e recursos, consultelink:hu-nvme-index.html["Visão geral do NVME-oF"^] .

*NVMe-oF com RHEL 9.6 tem as seguintes limitações conhecidas:*
...
.Passos
. Instale o RHEL 9.6 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel RHEL 9.6 necessário:
+
[source, cli]
----
uname -r
...
=== Modifique a iopolicy nas regras do udev

O RHEL 9.6 define a política de io padrão para NVMe-oF como `round-robin` .  Se você quiser alterar a iopolicy para `queue-depth` , modifique o arquivo de regras do udev da seguinte forma:
...
----
. Execute os seguintes comandos para recarregar as regras do udev e aplicar as alterações:
+
[source, cli]
----
...
----
. Verifique a iopolicy atual do seu subsistema.  Substitua <subsistema> pelo nome apropriado, por exemplo, `nvme-subsys0` .
+
[source, cli]
----
...
A partir do ONTAP 9.12.1, a autenticação segura em banda é suportada por NVMe/TCP entre um host RHEL 10.0 e um controlador ONTAP .
