...
summary: Como configurar o host NVMe-oF para RHEL 10.0 com ONTAP 
---
= Configurar RHEL 10.0 para NVMe-oF com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe sobre Fabrics (NVMe-oF) para RHEL 10.0.  Para obter mais informações sobre suporte e recursos, consultelink:hu-nvme-index.html["Visão geral do NVME-oF"^] .

*NVMe-oF com RHEL 10.0 tem as seguintes limitações conhecidas:*
...
.Passos
. Instale o RHEL 10.0 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel RHEL 10.0 necessário:
+
[source, cli]
----
uname -r
...
cat /etc/nvme/hostnqn
----
. Gere a chave dhchap para o host RHEL 10.0.
+
