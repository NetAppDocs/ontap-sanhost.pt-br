...
summary: Como configurar um host NVMe-oF para RHEL 9x com ONTAP 
---
= Configure o RHEL 9x para NVMe-oF com armazenamento ONTAP.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para RHEL 9x. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rhel-supported-features.html["Suporte e recursos do RHEL ONTAP"].

O NVMe-oF com RHEL 9x apresenta as seguintes limitações conhecidas:
...
.Passos
. Instale o RHEL 9x no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel RHEL 9x necessário:
+
[source, cli]
----
uname -r
...
A autenticação segura em banda é suportada via NVMe/TCP entre um host RHEL 9x e um controlador ONTAP .
...
cat /etc/nvme/hostnqn
----
. Gere a chave dhchap para o host RHEL 9x.
+
