...
[NOTE]
====
Se você pretende executar o tráfego NVMe e SCSI no mesmo host Oracle Linux 8,7, a NetApp recomenda o uso do multipath NVMe no kernel para namespaces ONTAP e dm-multipath para LUNs ONTAP, respetivamente. Isso também significa que os namespaces ONTAP devem ser listados na lista negra em dm-multipath para evitar que dm-multipath reivindique esses dispositivos de namespace. Você pode fazer isso adicionando a `enable_foreign` configuração ao `/etc/multipath.conf` arquivo:
