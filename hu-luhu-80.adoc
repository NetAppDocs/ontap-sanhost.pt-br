...
.Sobre esta tarefa
Ao instalar o Linux Host Utilities, isso não altera nenhuma configuração ou comportamento do seu host.
...
NOTE: O Linux Host Utilities tem uma dependência de instalação na biblioteca libHBAAPI.so. Você precisa instalar um dos pacotes listados acima, mesmo que um host seja destinado a uma configuração somente iSCSI. Como alternativa, você pode usar ferramentas em nível de sistema operacional para gerenciamento de LUN e caminhos.
