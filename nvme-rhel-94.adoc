...
---
= Configurar RHEL 9.4 para NVMe-oF com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda a configurar hosts NVMe sobre Fabrics (NVMe-oF) para RHEL 9.4.  Para obter mais informações sobre suporte e recursos, consultelink:hu-nvme-index.html["Visão geral do NVME-oF"^] .

O NVMe-oF com RHEL 9.4 não tem limitações conhecidas.
...
.Passos
. Instale o RHEL 9.4 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel RHEL 9.4 necessário:
+
[source, cli]
----
uname -r
...
A partir do ONTAP 9.12.1, a autenticação segura em banda é suportada por NVMe/TCP entre um host RHEL 9.4 e um controlador ONTAP .
...
cat /etc/nvme/hostnqn
----
. Gere a chave dhchap para o host RHEL 9.4.
+
