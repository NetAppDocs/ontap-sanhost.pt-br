...
Você pode usar o `multipath -ll` comando para verificar as configurações dos LUNs do ONTAP. As seções a seguir fornecem saída multipath de amostra para personas não-ASA mapeadas pelo LUN.
