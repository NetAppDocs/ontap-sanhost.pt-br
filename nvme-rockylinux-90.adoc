...
---
= Configurar o Rocky Linux 9.0 para NVMe-oF com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.0. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rockylinux-supported-features.html["Suporte e recursos do Rocky Linux ONTAP"].

*NVMe-oF com Rocky Linux 9.0 tem as seguintes limitações conhecidas:*
...
.Passos
. Instale o Rocky Linux 9.0 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel Rocky Linux 9.0 necessário:
+
[source, cli]
----
uname -r
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Hosts Rocky Linux 9.0 NVMe-oF criam controladores de descoberta persistentes duplicados | Em hosts NVMe-oF, você pode usar o comando "nvme discover -p" para criar Controladores de Descoberta Persistentes (PDCs).  Quando este comando é usado, apenas um PDC deve ser criado por combinação iniciador-alvo.  Entretanto, se você estiver executando o ONTAP 9.10.1 e o Rocky Linux 9.0 com um host NVMe-oF, um PDC duplicado será criado sempre que "nvme discover -p" for executado.  Isso leva ao uso desnecessário de recursos tanto no host quanto no destino. 
|===