...
summary: Configuração de host NVMe-oF para Oracle Linux 9x com ONTAP 
---
= Configure o Oracle Linux 9x com NVMe-oF para armazenamento ONTAP.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Oracle Linux 9x. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-ol-supported-features.html["Suporte e recursos do Oracle Linux ONTAP"].

O NVMe-oF com Oracle Linux 9x apresenta as seguintes limitações conhecidas:
...
Utilize o procedimento a seguir para validar as versões mínimas de software suportadas pelo Oracle Linux 9x.

.Passos
. Instale o Oracle Linux 9x no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel Oracle Linux 9x especificado.
+
[source, cli]
----
uname -r
...
----
. No host Oracle Linux 9x, verifique o `hostnqn` corda em `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
A autenticação segura em banda é suportada via NVMe/TCP entre um host Oracle Linux 9x e um controlador ONTAP .
