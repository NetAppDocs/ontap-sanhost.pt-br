...
[role="lead"]
O NVMe-of (incluindo NVMe/FC e NVMe/TCP) é compatível com o RHEL 9,0 com acesso de namespace assimétrico (ANA) necessário para sobreviverem a failovers de storage (SFOs) no array ONTAP. ANA é o equivalente a ALUA no ambiente NVM-of e atualmente é implementado com o NVMe Multipath no kernel. Este documento contém os detalhes para habilitar o NVMe-of com o Multipath NVMe no kernel usando ANA no RHEL 9,0 e ONTAP como destino.
...
.Marvell/QLogic
--
O driver nativo da caixa de entrada qla2xxx incluído no kernel RHEL 9,0 tem as correções mais recentes, essenciais para o suporte ao ONTAP.
