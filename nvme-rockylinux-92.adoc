...
summary: Como configurar o host NVMe-oF para Rocky Linux 9.2 com ONTAP 
---
= Configurar o Rocky Linux 9.2 para NVMe-oF com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.2. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rockylinux-supported-features.html["Suporte e recursos do Rocky Linux ONTAP"].

*NVMe-oF com Rocky Linux 9.2 tem as seguintes limitações conhecidas:*
...
.Passos
. Instale o Rocky Linux 9.2 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel necessário do Rocky Linux 9.2:
+
[source, cli]
----
uname -r
