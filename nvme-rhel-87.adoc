...
Aprenda a configurar hosts NVMe sobre Fabrics (NVMe-oF) para RHEL 8.7.  Para obter mais suporte e informações sobre recursos, consultelink:hu-nvme-index.html["Visão geral do NVME-oF"^] .
...
.Passos
. Instale o RHEL 8.7 no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel RHEL 8.7 necessário:
+
[source, cli]
----
uname -r
...
----
. No host RHEL 8.7, verifique o `hostnqn` corda em `/etc/nvme/hostnqn` :
+
[source, cli]
----
