...
summary: Descreve como usar o Oracle Linux 8,4 com ONTAP 
---
= Use o Oracle Linux 8,4 com ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Você pode usar as configurações de host SAN ONTAP para configurar o Oracle Linux 8,4 com ONTAP como destino.
...
Para Oracle Linux (OL) 8,4 o arquivo /etc/multipath.conf deve existir, mas você não precisa fazer alterações específicas no arquivo. O OL 8,4 é compilado com todas as configurações necessárias para reconhecer e gerenciar corretamente LUNs ONTAP.
...
O sistema operacional Oracle Linux 8,4 é compilado para reconhecer LUNs ONTAP e definir automaticamente todos os parâmetros de configuração corretamente para configurações ASA e não-ASA.
...
NOTE: Para configurar o Oracle Linux 8,4 Red Hat Enterprise Kernel (RHCK), use o link:hu_rhel_84.html#recommended-settings["definições recomendadas"] para Red Hat Enterprise Linux (RHEL) 8,4.
...
Não há problemas conhecidos para o Oracle Linux 8,4 com a versão ONTAP.


NOTE: Para problemas conhecidos do Oracle Linux (kernel compatível com Red Hat), consulte a link:hu_rhel_84.html#known-problems-and-limitations["problemas conhecidos"] seção na documentação de lançamento do Red Hat Enterprise Linux correspondente.
