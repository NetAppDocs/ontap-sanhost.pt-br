...
---
= Solução de problemas de armazenamento NVMe-oF com ONTAP para utilitários de host Linux
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Solucionar problemas de falhas NVMe-oF em hosts Oracle Linux, RHEL, Rocky Linux e SUSE Linux Enterprise Server com armazenamento ONTAP .

Antes de iniciar a resolução de problemas, verifique se você está executando uma configuração compatível com o https://mysupport.netapp.com/matrix/#welcome["Ferramenta de Matriz de interoperabilidade"^] especificações e, em seguida, prossiga com as próximas etapas para depurar quaisquer problemas do lado do host.


NOTE: As instruções de resolução de problemas não se aplicam a hosts AIX, ESXi, Proxmox e Windows.
...
.Passos
Ative o registro detalhado para LPFC ou Qlogic (Qla2xxx).
...
== Erros comuns do "nvme-cli" e soluções alternativas
...
Caso os problemas persistam, colete os seguintes arquivos e resultados de comandos e entre em contato link:https://mysupport.netapp.com["Suporte à NetApp"^] para uma análise mais detalhada:
