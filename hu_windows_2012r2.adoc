...
summary: Instale os Utilitários de Host do Windows em um host Windows Server 2012 R2 para ajudar no gerenciamento das operações dos protocolos FCP e iSCSI com LUNs ONTAP . 
---
= Configurar o Windows Server 2012 R2 para FCP e iSCSI com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Os utilitários de host do Windows permitem conectar hosts Windows a discos virtuais (LUNs) em um SAN NetApp .  Instale os Utilitários de Host do Windows em um host Windows Server 2012 R2 para ajudar no gerenciamento das operações dos protocolos FCP e iSCSI com LUNs ONTAP .
