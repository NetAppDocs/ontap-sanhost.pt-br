...
summary: Como configurar o host NVMe-oF para Rocky Linux 8.9 com ONTAP 
---
= Configurar o Rocky Linux 8.9 com NVMe-oF para armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Hosts Rocky Linux 8.9 suportam os protocolos NVMe/FC e NVMe/TCP com Acesso Assimétrico ao Namespace (ANA). ANA é equivalente ao multipathing de acesso assimétrico à unidade lógica (ALUA) em ambientes iSCSI e FCP e é implementado usando o recurso multipath NVMe no kernel.
...
.Sobre esta tarefa
Você pode usar o seguinte suporte e recursos com a configuração do host NVMe-oF para o Rocky Linux 8.9. Você também deve rever as limitações conhecidas antes de iniciar o processo de configuração.
...
+
** O multipath NVMe no kernel é desabilitado por padrão para hosts Rocky Linux 8.9 NVMe-oF. Portanto, você precisa ativá-lo manualmente.
** Em hosts Rocky Linux 8.9, NVMe/TCP é um recurso de prévia de tecnologia devido a problemas em aberto. Consulte https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/8/html-single/8.9_release_notes/index#technology-preview_file-systems-and-storage["Notas de lançamento do Rocky Linux 8.9"^] para obter mais informações.
...
Use o procedimento a seguir para validar as versões mínimas suportadas do software Rocky Linux 8.9.

.Passos
. Instale o Rocky Linux 8.9 no servidor. Após a conclusão da instalação, verifique se você está executando o kernel especificado do Rocky Linux 8.9:
+
[source, cli]
----
uname -r
...
A configuração do host NVMe-oF para o Rocky Linux 8.9 com versão de armazenamento ONTAP tem o seguinte problema conhecido:
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1479047["1479047"^] | Hosts Rocky Linux 8.9 NVMe-oF criam controladores de descoberta persistentes duplicados | Em hosts NVMe over Fabrics (NVMe-of), você pode usar o comando "nvme Discover -p" para criar PDCs (Controladoras de descoberta persistentes). Quando este comando é usado, apenas um PDC deve ser criado por combinação iniciador-alvo. Entretanto, se você estiver executando o Rocky Linux 8.9 em um host NVMe-oF, um PDC duplicado será criado toda vez que "nvme discover -p" for executado. Isso leva ao uso desnecessário de recursos no host e no destino. 
|===