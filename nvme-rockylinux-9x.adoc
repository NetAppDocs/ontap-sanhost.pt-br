...
summary: Como configurar um host NVMe-oF para Rocky Linux 9.x com ONTAP 
---
= Configure o Rocky Linux 9.x para NVMe-oF com armazenamento ONTAP.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 9.x. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rockylinux-supported-features.html["Suporte e recursos do Rocky Linux ONTAP"].

O NVMe-oF com Rocky Linux 9.x apresenta as seguintes limitações conhecidas:
...
== Passo 2: Instale o Rocky Linux e o software NVMe e verifique sua configuração.
...
.Passos
. Instale o Rocky Linux 9.x no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel Rocky Linux 9.x necessário:
+
[source, cli]
----
uname -r
...
----
=====
. Verifique se as outras combinações de LIF de iniciador-alvo NVMe/TCP podem obter com sucesso os dados da página de log de descoberta:
+
[source, cli]
----
nvme discover -t tcp -w host-traddr -a traddr
...
A autenticação segura em banda é suportada via NVMe/TCP entre um host Rocky Linux 9x e um controlador ONTAP .
...
cat /etc/nvme/hostnqn
----
. Gere a chave dhchap para o host Rocky Linux 9.x.
+
