...
Se o seu host estiver conectado a LUNs de outros fornecedores e qualquer uma das configurações de parâmetros de multipath tiver sido sobrescrita, você precisa corrigi-las adicionando trechos no arquivo  `multipath.conf` que se aplicam especificamente às LUNs do ONTAP. Você deve rolar o arquivo para baixo para adicionar esses trechos. Se você não corrigir os parâmetros, as LUNs do ONTAP podem não funcionar como esperado.
