...
summary: Instale o Linux Host Utilities em um host Oracle Linux 7.1 para gerenciar operações de protocolo FCP e iSCSI com LUNs ONTAP. 
---
= Configurar o Oracle Linux 7.1 para FCP e iSCSI com armazenamento ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
O software Utilitários de host Linux fornece ferramentas de gerenciamento e diagnóstico para hosts Linux que estão conetados ao storage ONTAP. Ao instalar o Linux Host Utilities em um host Oracle Linux 7.1, você pode usá-lo para ajudar a gerenciar operações de protocolo FCP e iSCSI com LUNs ONTAP.
...
Você pode usar multipathing com o Oracle Linux 7,1 para gerenciar LUNs ONTAP. O Oracle Linux 7,1 suporta Unbreakable Enterprise Kernel (UEK) R3 e UEK R4. O sistema operacional é inicializado com o kernel UEK R3 por padrão.


NOTE: Você pode usar o link:hu_rhel_71.html#rhel-rhck["configurações recomendadas para Red Hat Enterprise Linux (RHEL) 7.1"] para configurar o Red Hat Compatible Kernel para Oracle Linux 7.1.
