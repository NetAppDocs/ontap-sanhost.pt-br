...
summary: Descreve como usar o Oracle Linux 8,6 com ONTAP 
---
= Use o Oracle Linux 8,6 com ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
Você pode usar as configurações de host SAN ONTAP para configurar o Oracle Linux 8,6 com ONTAP como destino.
...
Para o Oracle Linux (OL) 8,6 o `/etc/multipath.conf` arquivo deve existir, mas você não precisa fazer alterações específicas no arquivo. O OL 8,6 é compilado com todas as configurações necessárias para reconhecer e gerenciar corretamente LUNs ONTAP.
...
O sistema operacional Oracle Linux 8,6 é compilado para reconhecer LUNs ONTAP e definir automaticamente todos os parâmetros de configuração corretamente para configurações ASA e não-ASA.
...
NOTE: Para configurar o Oracle Linux 8,6 Red Hat Enterprise Kernel (RHCK), use o link:hu_rhel_86.html#recommended-settings["definições recomendadas"] para Red Hat Enterprise Linux (RHEL) 8,6.
...
Não há problemas conhecidos para o Oracle Linux 8,6 com a versão ONTAP.


NOTE: Para problemas conhecidos do Oracle Linux (kernel compatível com Red Hat), consulte a link:hu_rhel_86.html#known-problems-and-limitations["problemas conhecidos"] seção na documentação de lançamento do Red Hat Enterprise Linux correspondente.
