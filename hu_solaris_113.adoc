...
** NetApp suporta o uso das tecnologias de virtualização Solaris quando a configuração geral é suportada pela Oracle e qualquer partição com acesso direto a LUNs está listada na link:https://imt.netapp.com/matrix/#welcome["IMT"^] em uma configuração suportada. Isso inclui contêineres raiz, domínios de E/S LDOM e LDOM usando NPIV para acessar LUNs.
