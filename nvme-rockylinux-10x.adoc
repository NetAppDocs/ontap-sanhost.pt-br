...
summary: Como configurar um host NVMe-oF para Rocky Linux 10.x com armazenamento ONTAP 
---
= Configure o Rocky Linux 10.x para NVMe-oF com armazenamento ONTAP.
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Rocky Linux 10.x. Para obter mais informações sobre suporte e funcionalidades, consulte link:nvme-rockylinux-supported-features.html["Suporte e recursos do Rocky Linux ONTAP"].

O NVMe-oF com Rocky Linux 10.x apresenta as seguintes limitações conhecidas:
...
.Passos
. Instale o Rocky Linux 10.x no servidor.  Após a conclusão da instalação, verifique se você está executando o kernel Rocky Linux 10.x necessário:
+
[source, cli]
----
uname -r
...
A autenticação segura em banda é suportada via NVMe/TCP entre um host Rocky Linux 10.x e um controlador ONTAP .
...
cat /etc/nvme/hostnqn
----
. Gere a chave dhchap para o host Rocky Linux 10.x.
+
