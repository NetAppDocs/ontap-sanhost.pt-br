...
---
= Configurar o Ubuntu 22.04 para NVMe-oF com storage ONTAP
:allow-uri-read: 
...
Aprenda como configurar hosts NVMe over Fabrics (NVMe-oF) para Ubuntu 22.04. Para obter mais informações sobre suporte e recursos, consulte link:nvme-ubuntu-supported-features.html["Suporte e recursos do Ubuntu ONTAP"].

NVMe-oF com Ubuntu 22.04 apresenta as seguintes limitações conhecidas:

* A inicialização SAN usando o protocolo NVMe-oF atualmente não é suportada para Ubuntu 22.04 com ONTAP.


Para obter detalhes adicionais sobre as configurações suportadas, consulte o link:https://support.netapp.com/matrix/["Ferramenta de Matriz de interoperabilidade"^].
...
.NVMe/TCP
--
O NVMe/TCP não possui funcionalidade de conexão automática. Em vez disso, você pode descobrir os subsistemas e namespaces do NVMe/TCP executando as operações NVMe/TCP  `connect` ou  `connect-all` manualmente.

Se um caminho for interrompido e não for restabelecido dentro do período de tempo limite padrão de 10 minutos, NVMe/TCP não poderá se reconectar automaticamente. Para evitar um tempo limite, defina o período de novas tentativas para eventos de failover para pelo menos 30 minutos.
